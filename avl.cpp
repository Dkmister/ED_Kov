...
pNodoA* InsereAVL (pNodoA *a, TipoInfo x, char *palavra, int *ok)
{
/* Insere nodo em uma árvore AVL, onde A representa a raiz da árvore,
  x, a chave a ser inserida e h a altura da árvore */
     ...
     if (x < a->info)
     {
		a->esq = InsereAVL(a->esq,x,palavra,ok);
        if (*ok)
        {
   		    switch (a->FB) {
     ...
     {
  		    a->dir = InsereAVL(a->dir,x,palavra,ok);
            if (*ok)
            {
              switch (a->FB) {
                case  1:  a->FB = 0; *ok = 0; break;
...
/*
busca => recebe um arvore, uma palavra e um arquivo.
...
0 - Caso contrário
*/
int busca(pNodoA* arv, char *palavra2, FILE* arq)
{
    ///achou = 0;
    if(arv == NULL)
    {
    ...
    {
        if(strncasecmp(arv->palavra, palavra2, strlen(palavra2))==0)
        {
            busca(arv->dir, palavra2, arq);
            busca(arv->esq, palavra2, arq);
            fprintf(arq, "         %lld %s\n", arv->info, arv->palavra);
            return 1;
        }
        else
        {
            achou = busca(arv->dir, palavra2, arq);
            achou = busca(arv->esq, palavra2, arq);
